...
def neg_sample(pos_item,item_size):
    neg_item = np.random.randint(0, item_size)
    while neg_item == pos_item:  # 确保负样本和正样本不同
        neg_item = np.random.randint(0, item_size)
    return neg_item 
...
'''def set_all_seeds(seed):
    ...
    torch.cuda.manual_seed(seed) # PyTorch CUDA随机库 (当前设备)
    torch.cuda.manual_seed_all(seed) # PyTorch CUDA随机库 (所有设备)
    transformers_set_seed(seed) # Transformers库随机库

    # 确保PyTorch的CUDA行为具有确定性
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.enabled = True
    torch.use_deterministic_algorithms(False)

    # 设置CUDA的工作空间配置,有助于确保模型的可重复性
...
def ndcg_last_timestep(y_true_seq, y_pred_seq, k):
    ...
    rel = 0
    for i in range(k):
        if top_k_rec[i] == y_true:
            rel = 1
            break

    dcg_k += rel / np.log2(i + 2)
